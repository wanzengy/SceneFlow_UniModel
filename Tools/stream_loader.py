...
class H5Stream(object):
    ...
    def __iter__(self):  
        ...
        for cur_idx in self.idx:
            ...
            timesurface = encoder.events_to_timesurface(xs, ys, ts, ps, 1, self.resolution, polarity=True)
...
class H5Dataloader(StreamDataLoader):
    ...
    def get_pair_event_info(self, fname, mode, window):
        file = h5py.File(fname, "r")
        cur_ts, last_ts = 0, 0
        maps = None
        t0 = file.attrs.get("t0", 0)
        if mode in ["images", "optical_flow", "scene_flow"]:
            maps = Map()
            name_prefix = {"depth":"depth/D_", "flow":"flow_dt1/F_", "flow_dt4":"flow_dt4/F_"}
            file[].visititems(maps)
            last_ts = len(maps.ts)
        elif mode == "time":
            last_left_ts = file["left/events"][-1]['t']
            last_right_ts = file["right/events"][-1]['t']
        else:
            last_left_ts = len(file["left/events"])
            last_right_ts = len(file["right/events"])

        left_events = file["/left/events"]
        right_events = file["right/events"]
        left_idx = []
        right_idx = []
        syn_time = left_events['t'][0]
        
        cur_left_idx = 0
        cur_right_idx = 0

        while self.check_seq(mode, cur_left_idx, last_left_ts, window) and \
                self.check_seq(mode, cur_right_idx, last_right_ts, window):
            idx0, idx1 = self.get_event_index(mode, left_events, syn_time, cur_left_idx, window=window, time_maps=maps.time, ms_to_idx=file["left/event_ms_to_idx"])
            left_idx.append((idx0, idx1))
            cur_left_idx = idx1

            idx0, idx1 = self.get_event_index(mode, right_events, syn_time, cur_right_idx, window=window, time_maps=maps.time, ms_to_idx=file["right/event_ms_to_idx"])
            right_idx.append((idx0, idx1))
            cur_right_idx = idx1

            syn_time = left_events['t'][idx1]
    ...
    def get_event_index(self, mode, events, syn_time, cur_idx, window=0, time_maps=None, ms_to_idx=None):
        """
        ...
        event_idx0 = None
        event_idx1 = None
        if abs(events[cur_idx]['t'] - syn_time) > 1e-4:
            cur_idx = ms_to_idx[np.floor((syn_time - events[0]['t']) * 1000)]
            while events[cur_idx]['t'] < syn_time:
                cur_idx += 1

        if mode == "events":
            event_idx0 = cur_idx
            event_idx1 = cur_idx + int(window)
        
        elif mode == "time":
            cur_ts = events[cur_idx]['t']
            
            event_idx0 = ms_to_idx[np.floor((cur_ts - events[0]['t']) * 1000)]
            event_idx1 = ms_to_idx[np.floor((cur_ts - events[0]['t'] + window) * 1000)]

            while events[event_idx0]['t'] < cur_ts:
                event_idx0 += 1

            while events[event_idx1]['t'] < (cur_ts + window):
                event_idx1 += 1
            ...
            cur_ts = time_maps[idx0] - events[0]['t']
            event_idx0 = ms_to_idx[np.floor((cur_ts  - events[0]['t']) * 1000)]
            while events[event_idx0]['t'] < cur_ts:
                event_idx0 += 1

            cur_ts = time_maps[idx1]
            event_idx1 = ms_to_idx[np.floor((cur_ts - events[0]['t']) * 1000)]
            while events[event_idx1]['t'] < cur_ts:
                event_idx1 += 1
