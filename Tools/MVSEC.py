...
class H5Stream(object):
    ...
    def __iter__(self):  
        ...
        for i in range(len(self.stereo_idx[0])):
            output = {'left':self.load_events(self.left_events, self.stereo_idx[0][i]), 
                    'right':self.load_events(self.right_events, self.stereo_idx[1][i])}
            ...
            if self.get_gt:
                output['gt'] = {}
                output['gt']['depth'] = torch.from_numpy(self.gt_file[f"depth/D_{self.stereo_idx[2][i]}"][:])
                output['gt']['lin_vel'] = torch.from_numpy(self.gt_file[f'lin_vel'][self.stereo_idx[2][i]][:])
                output['gt']['ang_vel'] = torch.from_numpy(self.gt_file[f'ang_vel'][self.stereo_idx[2][i]][:])
                output['gt']['of'] = torch.from_numpy(self.gt_file[f"optical_flow/F_{self.stereo_idx[2][i]}"][:])
...
class H5Dataloader(StreamDataLoader):
    def __init__(self, 
        ...
        def iterator_func(file_name):
            ...
            return H5Stream(file_name, idx, window, resolution, num_bins, augmentation, augment_prob, get_gt)
