...
def train(cfg, net, logger, dataset='UZHFPV'):
    ...
    from Tools.MVSEC_2 import H5Dataloader as Dataloader
    ...
    for epoch in range(cfg['Data']['num_epochs']):
        ...
        for i, item in enumerate(loader, start=1):
            ...
            # print(f"loader {timer.elapsed:.4f}")
                ...
                for loc in ['left', 'right']:
                    ...
                    if cfg['Loss']['overwrite_intermediate']:
                        loss_func.overwrite_intermediate(out[loc]['disps'],
                                                        out[loc]['ego_motion'],
                                                        loc=loc)
                # print(f"process {timer.elapsed:.4f}")
                loss_output = loss_func()
                loss = loss_output['loss']
                left_iwe = loss_output['left_iwe']
                loss_left_terms = loss_output['loss_left_terms']

            # for j in range(len(out['left']['disps'])):
            #     out['left']['disps'][j].register_hook(check_grad(f'left_disp_{j}'))
            ...
            # ---------- Log and Visualize ----------
            if cfg['Rec']['visdom'] and i % (cfg['Data']['seq_len'] * 10) == 0:
            # if cfg['Rec']['visdom']:
                ...
                pred_depth = loss_func.focal / (out['left']['disps'][-1]  * loss_func.flow_scaling  + 1e-9) * loss_func.baseline 
                pred_depth = pred_depth.detach().cpu()
                # pred_depth[out['left']['disps'][-1] > 1e-3] = 0
                # pred_depth[out['left']['disps'][-1] < 1e-3] = 0
                ...
                mask = torch.isfinite(gt_depth) & (gt_depth > 0) & (pred_depth > 0)
                ...
                plotter.vis_curve(X=np.array([loss_left_terms['temp'].item()]), Y=np.array([n_iter]), win='temporal curve')
                plotter.vis_curve(X=np.array([loss_left_terms['smooth'].item()]), Y=np.array([n_iter]), win='smoothness curve')
                plotter.vis_curve(X=np.array([loss_left_terms['consistensy'].item()]), Y=np.array([n_iter]), win='consistensy curve')
                plotter.vis_curve(X=np.array([loss_left_terms['stereo'].item()]), Y=np.array([n_iter]), win='stereo curve')
                n_iter += 1

            # if dataset in ['DSEC']:
            infor = f'{i:04d} / {len(loader):04d} '
            # else:
            ...
            print(infor + f'{timer.elapsed:.4f} seconds/batch', end="\r",)
