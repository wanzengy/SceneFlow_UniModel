...
class Model(BaseModel):
    ...
    def __init__(self, num_bins, base_num_channels, kernel_size, encoding, mask_output, activations, norm_input=False, **kwargs):
        ...
        self.head = self.head_neuron(1, base_num_channels, kernel_size, activation=ff_act, **self.kwargs[0])

        self.G1 = self.rec_neuron(
            base_num_channels, 1, kernel_size, activation=ff_act, **self.kwargs[1]
        )
        self.R1a = self.ff_neuron(
            2, base_num_channels, kernel_size, activation=ff_act, **self.kwargs[2]
        )
    ...
    def forward(self, event_voxel, event_cnt, log=False):
        ...
        time_surface = torch.nn.functional.pad(x2, (1, 0, 1, 0), "constant", 0)
        init_x = time_surface[..., 1:, 1:] - time_surface[..., :-1, 1:]
        init_y = time_surface[..., 1:, 1:] - time_surface[..., 1:, :-1]
        # mask = (time_surface[..., 1:, 1:] > 0) & (time_surface[..., 1:, :-1] > 0) & (time_surface[..., :-1, 1:] > 0)
        x2 = torch.concat([init_x, init_y], dim=1)
        x2 = 1 / (x2 + 1e-9)
        ...
        return {"flow": [flow], "activity": activity, 'timesurface': time_surface[..., :-1, :-1]}